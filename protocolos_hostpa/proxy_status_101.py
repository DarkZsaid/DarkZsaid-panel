...
# Respuesta exacta solicitada para WebSocket
...
class Server:
    ...
    def start(self):
        ...
        try:
            ...
            print(f"🎯 Destino fijo: {SSH_HOST}:{SSH_PORT}")
            
            while True:
                client, addr = self.soc.accept()
                # Uso de hilos moderno para Py3
    ...
    def handler(self, client, addr):
        ...
        try:
            # Recibir Payload para limpiar buffer
            ...
            if any(x in data_upper for x in [b"GET", b"POST", b"CONNECT", b"UPGRADE"]):
                ...
                # Enviar respuesta 101
                client.sendall(RESPONSE)
                
                # Iniciar puente de datos
